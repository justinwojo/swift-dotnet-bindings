...
#
# Validates BridgeParamTestApp on iOS Simulator
# Usage: ./validate.sh [timeout_seconds]
#
...
TIMEOUT=${1:-20}
APP_PATH="BridgeParamTestApp/bin/Debug/net10.0-ios/iossimulator-arm64/BridgeParamTestApp.app"
BUNDLE_ID="com.swiftbindings.bridgeparamtest"
...
count_crash_logs() {
    local files=( "$CRASH_LOG_DIR"/BridgeParamTestApp*.ips )
    if [[ -e "${files[0]}" ]]; then
        echo "${#files[@]}"
    else
        echo 0
...
if [ "$AFTER_CRASH_COUNT" -gt "$BEFORE_CRASH_COUNT" ]; then
    ...
    ls -t "$CRASH_LOG_DIR"/BridgeParamTestApp*.ips | head -1 | xargs head -50
    rm -f "$OUTPUT_FILE"
    exit 1
fi
