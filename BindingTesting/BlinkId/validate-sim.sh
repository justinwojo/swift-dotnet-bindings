...
BEFORE_CRASH_COUNT=$(ls -1 "$CRASH_LOG_DIR"/BlinkIdTestApp*.ips 2>/dev/null | wc -l || echo 0)
...
AFTER_CRASH_COUNT=$(ls -1 "$CRASH_LOG_DIR"/BlinkIdTestApp*.ips 2>/dev/null | wc -l || echo 0)
