#!/bin/bash
# Validates NukeTestApp on iOS Simulator
...
TIMEOUT=${1:-10}
APP_PATH="NukeTestApp/bin/Debug/net10.0-ios/iossimulator-arm64/NukeTestApp.app"
BUNDLE_ID="com.swiftbindings.nuketestapp"
CRASH_LOG_DIR="$HOME/Library/Logs/DiagnosticReports"

cd "$(dirname "$0")"

# Record crash log timestamps before running
BEFORE_CRASH_COUNT=$(ls -1 "$CRASH_LOG_DIR"/NukeTestApp*.ips 2>/dev/null | wc -l || echo 0)
...
AFTER_CRASH_COUNT=$(ls -1 "$CRASH_LOG_DIR"/NukeTestApp*.ips 2>/dev/null | wc -l || echo 0)
if [ "$AFTER_CRASH_COUNT" -gt "$BEFORE_CRASH_COUNT" ]; then
    echo "=== CRASH LOG DETECTED ==="
    ls -t "$CRASH_LOG_DIR"/NukeTestApp*.ips | head -1 | xargs head -50
    rm -f "$OUTPUT_FILE"
    exit 1
fi
