#!/bin/bash
# Validates CryptoSwiftTestApp on iOS Simulator
...
TIMEOUT=${1:-15}
APP_PATH="CryptoSwiftTestApp/bin/Debug/net10.0-ios/iossimulator-arm64/CryptoSwiftTestApp.app"
BUNDLE_ID="com.swiftbindings.cryptoswifttestapp"
CRASH_LOG_DIR="$HOME/Library/Logs/DiagnosticReports"

cd "$(dirname "$0")"

# Record crash log timestamps before running
BEFORE_CRASH_COUNT=$(ls -1 "$CRASH_LOG_DIR"/CryptoSwiftTestApp*.ips 2>/dev/null | wc -l || echo 0)
...
AFTER_CRASH_COUNT=$(ls -1 "$CRASH_LOG_DIR"/CryptoSwiftTestApp*.ips 2>/dev/null | wc -l || echo 0)
if [ "$AFTER_CRASH_COUNT" -gt "$BEFORE_CRASH_COUNT" ]; then
    echo "=== CRASH LOG DETECTED ==="
    ls -t "$CRASH_LOG_DIR"/CryptoSwiftTestApp*.ips | head -1 | xargs head -50
    rm -f "$OUTPUT_FILE"
    exit 1
fi
