...
///
/// PARITY CONTRACT WITH `SwiftInterfaceAccessParser.GetPublicTypeNames` (line 158):
///
/// 1. **Decl kinds emitted**: `class`, `struct`, `enum`, `actor`, `protocol`
///    declared with `public` or `open`. Internal/private types are NOT emitted
///    (the regex's `PublicTypeDeclRegex` only matches public/open).
///
/// 2. **Extensions are NOT emitted**: `extension Foo { … }` is scope-context only;
///    the extension target never enters the result. Mirrors the regex comment
///    "Extensions are for external module types and should not be treated as
...
///    `Foo<T>` — the regex captures `\w+` only.
///
/// 5. **Extension scope context**: when a public type is declared inside an
///    extension body, the extension target's first-dot-stripped form is
///    used as the prefix (matches `ExtensionDeclRegex` in the regex parser, which
///    pushes the qualified-after-first-dot name onto the type stack).
///
...
///    `Sub` is keyed via the full nesting `Outer.Hidden.Sub` because the regex
///    uses brace-depth tracking that still pushes internal types as scope.
///
/// 8. **Two-gate parity**: regex parser uses `TypeDeclRegex` (public|internal|open
///    + optional final) for the tracker's scope push and `PublicTypeDeclRegex`
///    (public|open + optional final) for emission. Modifiers like `indirect`
///    between access and the type keyword fail BOTH regexes — so neither push
///    nor emit happens. Types whose body opens on a later line also miss the
///    tracker's `openBraces > 0` gate, so they are not pushed either. We mirror
///    both gates here.
final class PublicTypeNamesWalker: SyntaxVisitor {
    ...
    /// matches the regex parser's `ExtensionDeclRegex` substring behavior).
    ...
    /// Two independent regex-parity gates:
    ///   * SCOPE PUSH gate: `TypeDeclRegex` (public|internal|open + optional final)
    ///     plus `openBraces > 0`. If this fails, the type is invisible to the
    ///     tracker — no push, no emit, no nesting prefix for its children.
    ...
    /// `TypeDeclRegex` shape (used by the tracker for scope-push):
    ...
    /// `PublicTypeDeclRegex` shape (used for emission):
    ...
    /// keyword. Anything else (`indirect`, `nonisolated`, etc.) means the regex
    /// would not match — so SwiftSyntax must skip emission too. Modifiers BEFORE
    /// access are tolerated (regex unanchored Match scan).
