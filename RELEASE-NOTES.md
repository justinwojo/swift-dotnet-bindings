...
Two changes here alter what already-compiling code does.
...
- **A property projected from a Swift `Int`/`UInt` reports a value it cannot represent instead of wrapping it, and on a concrete type a lossless sibling now sits beside it.** These properties are surfaced as `int`/`uint` so that idiomatic C# code reads naturally, but the underlying Swift value is pointer-width: a value above `int.MaxValue`/`uint.MaxValue` used to come back silently truncated — a 64-bit hash could read as a small number, and an `OptionSet` bit above 31 simply disappeared. Such a read now throws `OverflowException`, and where a companion accessor exists the message names it. That companion is the property's name with a `Native` suffix (`SizeLimit` → `SizeLimitNative`), typed `nint`/`nuint`, settable wherever the property is, and it never converts. `OptionSet` operators and `Contains` read the native-width accessor, so combining and testing options above bit 31 works as before. Values that fit the narrow type are unaffected, and method return types are unchanged — they were already pointer-width. **One read has no companion yet:** the same property reached through a generated protocol interface — an `any P` existential or a proxy standing in for a Swift conformer — also refuses to truncate, but the interface declares no `Native` member to fall back to, so the exception explains that the Swift declaration is pointer-width rather than naming an alternative. Reaching the value through the concrete type, where the companion is emitted, is the way around it for now.
