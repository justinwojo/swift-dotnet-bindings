...
public sealed class AppleTypesCsEmitter
{
    ...
    public void Emit(Manifest manifest, string outputDir)
    {
        ...
        var opaqueRegistrations = new List<OpaqueTypeRegistration>();
        ...
        foreach (var (moduleName, module) in manifest.Modules)
        {
            foreach (var entry in module.Types.OrderBy(t => t.SwiftIdentity, StringComparer.Ordinal))
            {
                try
                {
                    var registration = EmitEntry(moduleName, entry, outputDir);
                    if (registration is not null)
                        opaqueRegistrations.Add(registration);
                }
        ...
        EmitFrameworkResolverRegistration(outputDir, opaqueRegistrations);
    }

    // Emits a single file with a `[ModuleInitializer]` that validates the Runtime
    // dispatch contract, registers `SwiftFrameworkResolver`, and roots every emitted
    // VWT-opaque supplement type through concrete generic/type references. Without the
    // resolver, bare-name
    ...
    private void EmitFrameworkResolverRegistration(
        string outputDir,
        IReadOnlyList<OpaqueTypeRegistration> opaqueRegistrations)
    {
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        // Fail loudly before best-effort registrations if the loaded Runtime cannot");
        sb.AppendLine("        // speak the dispatch contract this supplement was generated against.");
        sb.AppendLine($"        RuntimeContract.AssertCompatible({ModuleHandler.EmittedRuntimeContractVersion});");
        sb.AppendLine("        SwiftFrameworkResolver.RegisterForAssembly(typeof(AppleSupplementRegistration).Assembly);");

        foreach (var registration in opaqueRegistrations
            .OrderBy(r => r.CSharpTypeName, StringComparer.Ordinal))
        {
            var factoryAndMetadata =
                $"try {{ global::Swift.Runtime.InteropServices.SwiftMarshal.RegisterSwiftObjectFactory<{registration.CSharpTypeName}>(); " +
                $"global::Swift.Runtime.SwiftObjectHelper<{registration.CSharpTypeName}>.GetTypeMetadata(); }} catch {{ }}";
            var payloadSemantics =
                $"try {{ global::Swift.Runtime.InteropServices.SwiftMarshal.RegisterPayloadSemantics(typeof({registration.CSharpTypeName}), " +
                "global::Swift.Runtime.PayloadConstructionSemantics.Copy); } catch { }";
            var body = factoryAndMetadata + " " + payloadSemantics;
            var guard = BuildIsAvailableCondition(registration.Availability);
            sb.AppendLine(guard is null
                ? $"        {body}"
                : $"        if ({guard}) {{ {body} }}");
        }
    ...
    private OpaqueTypeRegistration? EmitEntry(string moduleName, TypeEntry entry, string outputDir)
    {
        ...
        return useSequential
            ? null
            : new OpaqueTypeRegistration(GetCSharpTypeName(entry.ManagedProjection), accessor.Availability);
    }

    private static string GetCSharpTypeName(ManagedRef managedProjection)
        => "global::" + managedProjection.Namespace + "." +
            string.Join('.', managedProjection.DeclarationPath);

    private static string? BuildIsAvailableCondition(Availability availability)
    {
        if (availability is null || availability.IsEmpty)
            return null;

        var conditions = new List<string>();
        AddAvailabilityCondition(conditions, "ios", availability.Ios);
        AddAvailabilityCondition(conditions, "maccatalyst", availability.Maccatalyst);
        AddAvailabilityCondition(conditions, "tvos", availability.Tvos);
        AddAvailabilityCondition(conditions, "macos", availability.Macos);
        return conditions.Count == 0 ? "false" : string.Join(" || ", conditions);
    }

    private static void AddAvailabilityCondition(
        ...
        string? version)
    {
        ...
        if (!int.TryParse(parts[0], out var major))
            throw new EmitterGateException($"invalid availability version '{version}'");
        var minor = parts.Length > 1 && int.TryParse(parts[1], out var parsedMinor)
            ? parsedMinor
            : 0;
        var patch = parts.Length > 2 && int.TryParse(parts[2], out var parsedPatch)
            ? parsedPatch
            : 0;

        // The convenience IsIOSVersionAtLeast API intentionally returns true on Mac Catalyst.
        // Generated supplement guards must instead match the exact manifest platform before
        // checking its version, otherwise an ios-only entry can be warmed despite carrying
        // [UnsupportedOSPlatform("maccatalyst")].
        conditions.Add(
            $"(global::System.OperatingSystem.IsOSPlatform(\"{platform}\") && " +
            $"global::System.OperatingSystem.IsOSPlatformVersionAtLeast(\"{platform}\", {major}, {minor}, {patch}))");
    }

    private sealed record OpaqueTypeRegistration(string CSharpTypeName, Availability Availability);
