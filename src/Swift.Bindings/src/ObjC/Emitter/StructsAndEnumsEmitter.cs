...
public static class StructsAndEnumsEmitter
{
    ...
    public static StructsAndEnumsResult? Emit(ObjCModule module, string outputDir, string resolvedNamespace, ILogger logger, ObjCBindingDiagnostics? diagnostics = null, PlatformInfo? platformInfo = null, HashSet<string>? excludeTypeNames = null)
    {
        ...
        module = ObjCEmissionEligibility.Prepare(module, diagnostics, apiDefinition: false);
        blockTypedefs = module.Typedefs.Where(t => t.UnderlyingType.IsBlock).ToList();
    ...
    static void EmitEnum(StringBuilder sb, ObjCEnumDecl enumDecl, Dictionary<string, ObjCTypeRef>? typedefMap = null, ObjCBindingDiagnostics? diagnostics = null, string? moduleTag = null)
    {
        ...
        for (var i = 0; i < enumDecl.Cases.Count; i++)
        {
            ...
            var value = c.EvaluatedValue ?? (c.Value is { } legacy
                ? new ObjCIntegerValue(unchecked((ulong)legacy), false) : (ObjCIntegerValue?)null);
            var valueStr = value is { } integer ? $" = {FormatEnumValue(integer, baseType)}" : "";
    ...
    internal static string FormatEnumValue(ObjCIntegerValue value, string baseType)
    {
        var bits = value.Bits;
        // The declared enum carrier is authoritative, including casts evaluated by Clang.
        return baseType switch
        {
            "ulong" => bits.ToString(System.Globalization.CultureInfo.InvariantCulture) + (bits > long.MaxValue ? "UL" : ""),
            "uint" => unchecked((uint)bits).ToString(System.Globalization.CultureInfo.InvariantCulture),
            "ushort" => unchecked((ushort)bits).ToString(System.Globalization.CultureInfo.InvariantCulture),
            "byte" => unchecked((byte)bits).ToString(System.Globalization.CultureInfo.InvariantCulture),
            "sbyte" => unchecked((sbyte)bits).ToString(System.Globalization.CultureInfo.InvariantCulture),
            "short" => unchecked((short)bits).ToString(System.Globalization.CultureInfo.InvariantCulture),
            "int" => unchecked((int)bits).ToString(System.Globalization.CultureInfo.InvariantCulture),
            _ => unchecked((long)bits).ToString(System.Globalization.CultureInfo.InvariantCulture)
        };
    }
