...
public static class WorkaroundRecommendations
{
    ...
    public static string? GetRecommendation(SkipReason reason) => reason switch
    {
        ...
        SkipReason.PropertyWrapperDeclinedDirectPInvoke =>
            "The property IS emitted, but an accessor that remains on the direct Swift calling convention is listed separately under `DegradedMembers` with `DiagnosticId = DirectSwiftSelfExposure` when its final lowered signature carries untyped SwiftSelf. Read that row's public/accessor/native-call identity and reason before using the member on Mono full-AOT; exposure is allocator/workload dependent and is not by itself an observed crash. A Swift wrapper exposing the operation through a C-callable signature is the consumer workaround.",
        SkipReason.Unknown =>
