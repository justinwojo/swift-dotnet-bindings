...
/// swallowed it (downgrading to mangled-name substring heuristics such as
/// <c>DetectAsyncFromMangledName</c>). That made demangle-based async / convention / variadic
/// detection degrade invisibly: a single new node kind the reducer didn't handle disabled detection
/// for every symbol carrying it, with no diagnostic.
///
...
/// reachable miss across the BindingTests + validation ABI corpora. That gated channel is the
/// precondition for retiring the substring fallbacks in Finding 17 — a deletion can no longer
/// silently regress detection, because a reintroduced "No rule" hole turns the corpus test red.
///
...
internal static class ReductionDiagnostics
{
    ...
    /// here with the reason. Finding 17 tightens the gate by adding the <c>CFunctionPointer</c> rule
    /// and removing it from this set.
    /// </summary>
    public static readonly IReadOnlyDictionary<NodeKind, string> IntentionallyUnreducedKinds =
        new Dictionary<NodeKind, string>
        {
            ...
            // CFunctionPointer (@convention(c)) is unruled today; Finding 17 adds its rule and removes
            // this entry, which both fixes convention-c detection and tightens this gate.
            [NodeKind.CFunctionPointer] = "@convention(c) closure — unruled until Finding 17 adds its rule",
        };
