...
public static class MethodWrapperEmitter
{
    ...
    public static WrapperEligibility EvaluateWrapperEligibility(MethodEnvironment env)
    {
        ...
        // 11a. This ordinary synchronous producer owns initialized String storage through
        ...
        // 11b. Variadic parameters are supported via the unsafeBitCast bridge when the shape is
            ...
            switch (phase)
            {
                ...
                case CdeclPhase.Arguments:
                    for (int i = 0; i < keptArgs.Count; i++)
                    {
                        ...
                        // Bare external label for the wrapper's own signature ("_" for unlabeled). The
                        // shared recovery keeps a label that genuinely begins with '_' (e.g. _self) intact.
    ...
    private static List<ProtocolMethodParameter> GetProtocolMethodParameters(MethodDecl methodDecl)
    {
        ...
        for (int i = 0; i < keptArgs.Count; i++)
        {
            ...
            // Determine external label. The shared recovery keeps a label that genuinely begins
            // with '_' (e.g. _self) intact.
            string externalLabel;
            if (string.IsNullOrEmpty(arg.Name) || arg.Name == "_" || SwiftBuilder.IsAutoGeneratedArgName(arg.Name))
                externalLabel = "_";
            else
                externalLabel = NameProvider.RecoverSwiftArgumentLabel(arg);
    ...
    private static string EffectiveArgLabel(ArgumentDecl arg)
    {
        ...
        if (string.IsNullOrEmpty(name) || name == "_" || SwiftBuilder.IsAutoGeneratedArgName(name))
            return "_";
        // Same shared recovery CdeclParamMapper.BuildSwiftCallArgLabel uses, so `_self` and `self`
        // compare distinct here exactly as they are in Swift.
        return NameProvider.RecoverSwiftArgumentLabel(arg);
    }
