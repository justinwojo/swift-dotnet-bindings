...
/// Swift continuation box. Any escape resumes the box with an error via the C# resume scope so
/// Swift's task never hangs and the box is consumed exactly once. Wired by Finding 37.</item>
/// </list>
///
...
///
/// <para><see cref="EmitCloseAsyncWitnessFailFast"/> is a member-named refinement of the
/// <see cref="UcoFaultPolicy.FailFast"/> close for async protocol-requirement receivers. The
/// async witness is satisfied through the synchronously-blocked reverse-dispatch slot (the async
/// witness ABI hits the Mono reverse-async assertion, upstream Issue 1), which exposes no Swift
/// error channel — even for an <c>async throws</c> requirement — so any escaping exception is
/// still process-terminating. The refinement only makes the FailFast loud and attributable: a
/// cancellation-specific arm (<see cref="System.OperationCanceledException"/>, the routine C#
/// control-flow exception a token wired into the conformance would raise) and a general arm,
/// both naming the protocol member (Finding 36). The real async/error witness that would carry
/// the error back is Session 13.</para>
/// </summary>
public static class UcoGuardEmitter
{
        ...
        string[]? streamFaultBody = null)
    {
        ...
        switch (policy)
        {
            ...
            case UcoFaultPolicy.StreamFault:
                ...
                if (streamFaultBody is null || streamFaultBody.Length == 0)
                {
                    throw new global::System.ArgumentException(
                        "StreamFault policy requires a non-empty streamFaultBody (the catch statements).",
                        nameof(streamFaultBody));
                }
                writer.WriteLine($"catch (global::System.Exception {exceptionVar})");
                writer.WriteLine("{");
                writer.Indent++;
                foreach (var line in streamFaultBody)
                {
                ...
                break;
            default:
                // ResumeBoxError needs the async-closure resume scope (Finding 37 emits its own
                // envelope inline rather than routing through here). No caller passes it; this seam
                // makes the gap explicit rather than silently emitting a catch-free body.
                throw new global::System.NotImplementedException(
                    $"UCO fault policy {policy} is not yet wired (ResumeBoxError → Finding 37).");
        }
    }
    ...
    /// (Finding 36). The async witness blocks the C# <c>Task</c> on the synchronously-blocked
    ...
    /// Two catch arms are emitted (most-derived first, so the <see cref="System.OperationCanceledException"/>
    /// arm is reachable):
    /// <list type="number">
