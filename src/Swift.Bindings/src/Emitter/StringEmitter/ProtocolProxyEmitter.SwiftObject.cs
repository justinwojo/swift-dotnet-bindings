...
public partial class ProtocolProxyEmitter
{
    private void EmitISwiftObjectImplementation(CSharpWriter writer, ProtocolDecl protocolDecl, WitnessDispatchEmitter dispatchEmitter)
    {
        ...
        // direction. When EveryProtocolEmitter did NOT export a `SBW_Get_EveryProtocol_{P}_WitnessTable`
        ...
        var getWitnessTableBody = _witnessGetterEmitted
            ...
            : "// No " + EveryProtocolEmitter.GetWitnessTableGetterMangledName(protocolDecl) + " symbol was exported for this\n"
        ...
        var mangledName = EveryProtocolEmitter.GetSetVtableEntryPoint(modulePrefix: "", protocolDecl.Name);
        ...
        // The vtable-setter P/Invoke is only called from InitializeVtable's own-vtable population,
        // which runs only when the wrapper emitted the setter. A protocol with nothing to dispatch
        // (no requirements, static-only, noncopyable) gets no setter, and declaring it anyway names
        // an entry point no binary exports.
        ...
        // `SBW_Get_EveryProtocol_{P}_WitnessTable` symbol (read-only / class-superclass-skipped /
        ...
        if (_witnessGetterEmitted)
        {
            ...
            {
                LibraryPath = wrapperLibPath,
                EntryPoint = EveryProtocolEmitter.GetWitnessTableGetterMangledName(protocolDecl),
            ...
            {
                LibraryPath = wrapperLibPath,
                EntryPoint = EveryProtocolEmitter.GetExistentialSizeGetterMangledName(protocolDecl),
