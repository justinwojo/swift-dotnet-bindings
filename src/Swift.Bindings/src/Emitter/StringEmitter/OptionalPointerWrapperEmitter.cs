...
public static class OptionalPointerWrapperEmitter
{
        ...
        ModuleEmissionContext? emissionContext = null)
    {
        ...
        foreach (var arg in methodDecl.CSSignature.Skip(1))
        {
            ...
            // (resultPtr/_resultBuf/_self/errorOut) OR a sibling user binding before it becomes a
            ...
            // The setter value param (`newValue`, the first arg of any setter) IS the injected setter
            // synthetic — the assignment body references it by bare name via setterValueStr /
            // subscriptSetterValue, both computed from the UNescaped name. Reserved-escaping its
            // declaration (newValue → __newValue, since `newValue` is in the reserved set) would
            // diverge the declaration from the body → "cannot find 'newValue' in scope". This is the
            // escapeReservedCollision:false carve-out the Property/Subscript emitters already apply; user
            // index params (argIndex >= 1) keep the escape so a colliding index renames instead.
    ...
    /// so labels that genuinely begin with '_' (e.g. <c>_self</c>) are not corrupted by the
    /// legacy underscore-stripping recovery.
    /// </summary>
