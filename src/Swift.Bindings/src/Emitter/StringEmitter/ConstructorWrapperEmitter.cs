...
public static class ConstructorWrapperEmitter
{
        ...
        bool throws)
    {
        ...
        for (int i = 0; i < keptArgs.Count; i++)
        {
            ...
            // Bare external label ("_" for unlabeled). The shared recovery keeps a label that
            // genuinely begins with '_' (e.g. _self) intact.
        ...
        // Siblings MUST mirror the param-declaration loop above (CollectSiblingBindingNames(keptArgs))
        // so the reconstruction binding names match the @_cdecl param names exactly — a divergent
        // escape here would read a `{label}Val` from a parameter that was declared under a different
        // sibling-escaped name.
        var reconSiblings = CdeclParamMapper.CollectSiblingBindingNames(keptArgs);
        for (int i = 0; i < keptArgs.Count; i++)
        {
            var arg = keptArgs[i];
            if (DefaultParameterOverloadEmitter.IsDebugParameter(arg))
