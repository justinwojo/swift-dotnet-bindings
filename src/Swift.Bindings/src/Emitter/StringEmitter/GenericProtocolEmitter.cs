...
internal static class GenericProtocolEmitter
{
        ...
        bool throws)
    {
        ...
        for (int i = 0; i < keptArgs.Count; i++)
        {
            ...
            // Provenance-aware bare external label — prefer the parser-captured OriginalSwiftName
            // so a label that genuinely begins with '_' (e.g. _self) is not corrupted by the
            // legacy underscore strip.
