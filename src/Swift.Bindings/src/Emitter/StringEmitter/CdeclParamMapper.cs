...
public static class CdeclParamMapper
{
    ...
    /// positions. Recovers the raw Swift label via
    /// <see cref="NameProvider.RecoverSwiftArgumentLabel"/> and escapes via
    ...
    /// <see cref="ArgumentDecl.IsUnlabeledSubscriptIndex"/> emit no label at all.
    /// </summary>
    internal static string BuildSwiftCallArgLabel(ArgumentDecl arg)
    {
        ...
        // Shared provenance-aware recovery: the parser-captured original when present, otherwise
        // the keyword-gated un-escape. Either path yields the RAW Swift label, so a `default:`
        // label round-trips as `default:` — bare, because this is call-argument position (only
        // `inout` still needs backticks there) — and a label that genuinely starts with an
        // underscore survives intact.
        var swiftLabel = NameProvider.RecoverSwiftArgumentLabel(arg);
