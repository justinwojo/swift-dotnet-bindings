...
public class ProtocolProxyEmitter
{
    ...
    public void EmitProxyClass(CSharpWriter writer, ProtocolDecl protocolDecl)
    {
        ...
        {
            _logger.LogDebug($"Skipping proxy class for {protocolDecl.Name}: has Self requirement or associated types");
            return;
        }
        ...
        writer.WriteLine($"/// </summary>");
        writer.WriteLine($"public unsafe class {proxyClassName} : {interfaceName}, ISwiftObject");
        writer.WriteLine("{");
        writer.Indent++;
        ...
        EmitInstanceFields(writer, protocolDecl, interfaceName);
        ...
        EmitReceiverMethods(writer, protocolDecl, interfaceName);

        // Emit constructors
        EmitConstructors(writer, protocolDecl, interfaceName);

        // Emit interface implementation
        EmitInterfaceImplementation(writer, protocolDecl, interfaceName);
    ...
    private void EmitISwiftObjectImplementation(CSharpWriter writer, ProtocolDecl protocolDecl)
    {
        ...
        writer.WriteLines($$"""
            ...
            {
                [DllImport("{{_moduleName}}", CallingConvention = CallingConvention.Cdecl, EntryPoint = "{{mangledName}}")]
