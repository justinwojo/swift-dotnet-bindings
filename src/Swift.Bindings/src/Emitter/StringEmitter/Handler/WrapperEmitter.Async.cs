        ...
        private void EmitAsync(CSharpWriter csWriter, SwiftWriter swiftWriter)
        {
                ...
                .Select(p =>
                {
                    ...
                    if (usesCdecl)
                    {
                        ...
                        var (cdeclParam, reconstruction, _) =
                            CdeclParamMapper.Map(p, label, _env, omitLabels: true, reservedSiblings: asyncSiblings);
            ...
            var methodCallArgs = string.Join(", ", _env.MethodDecl.CSSignature.Skip(1)
                .Select(p =>
                {
                    ...
                    // CdeclParamMapper.Map escapes this binding when it collides with an injected
                    // synthetic (resultPtr/_self/…) OR a sibling user binding; mirror that here so
                    // the call references the same (possibly renamed) binding. No-op when there's no
                    // collision. Self-excluded from the sibling set (same as Map).
                    if (usesCdecl)
                        varName = NameProvider.EscapeReservedSwiftWrapperLabel(
                            varName, CdeclParamMapper.ExcludeSelf(asyncSiblings, varName));
                    ...
                    var argName = $"{CdeclParamMapper.BuildSwiftCallArgLabel(p)}{varName}";
                    ...
                    {
                        // Map names the reconstructed value `{escapedLabel}Val`, so escape here too
                        // (sibling-aware, self-excluded — same as Map) to find and reference the
                        // same line. No-op when there's no collision.
                        var rawLabel_ = !string.IsNullOrEmpty(p.PrivateName) ? p.PrivateName : p.Name;
                        var label_ = NameProvider.EscapeReservedSwiftWrapperLabel(
                            rawLabel_, CdeclParamMapper.ExcludeSelf(asyncSiblings, rawLabel_));
                        ...
                        {
                            // Provenance-aware call label (canonical builder) — preserves labels
                            // that genuinely begin with '_' (e.g. _self) and escapes keywords.
                            var argLabel = CdeclParamMapper.BuildSwiftCallArgLabel(p);
