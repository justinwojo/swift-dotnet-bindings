...
public static partial class CrossModuleExtensionEmitter
{
        ...
        bool isStatic)
    {
        ...
        // Call args with external labels matching the original method signature, via the
        // canonical builder (CdeclParamMapper.BuildSwiftCallArgLabel) rather than a local
        // auto-generated-name check — it also backtick-escapes a keyword-spelled label,
        // which the raw ArgumentDecl.Name here does not.
        ...
        for (int i = 1; i < method.CSSignature.Count; i++)
        {
            ...
            callArgsForMethod.Add($"{CdeclParamMapper.BuildSwiftCallArgLabel(argDecl)}{valueExpr}");
        }
        ...
        // synthesis) is READ, not called: `target.name`, no parens — mirrors the identical
        // struct-receiver fix in CrossModuleExtensionEmitter.Struct.cs.
        var methodSwiftName = NameProvider.EscapeSwiftKeyword(method.Name);
        var callExpr = method.IsExtensionPropertyGetter
            ? $"{dispatchTarget}.{methodSwiftName}"
            : $"{dispatchTarget}.{methodSwiftName}({string.Join(", ", callArgsForMethod)})";
        ...
        ILogger logger)
    {
        ...
        for (int i = 1; i < method.CSSignature.Count; i++)
        {
            ...
            parameters.Add(new AsyncTrampolineParamInfo
            {
                ...
                // Pre-rendered "label: " (or "" when unlabeled/keyword-escaped) via the canonical
                // builder — see the call-site note below for why the raw ArgumentDecl.Name is not
                // used directly.
                ArgLabel = CdeclParamMapper.BuildSwiftCallArgLabel(arg).TrimEnd(':', ' '),
            });
        }
        ...
        ITypeDatabase typeDatabase)
    {
        ...
        for (int i = 0; i < parameters.Count; i++)
        {
            ...
            // p.ArgLabel is already the canonical pre-rendered label (empty when unlabeled).
            callArgs.Add(string.IsNullOrEmpty(p.ArgLabel) ? converted : $"{p.ArgLabel}: {converted}");
        }
        ...
        // Same IsExtensionPropertyGetter carve-out as the sync class trampoline above — an
        // async/throwing extension-default property getter (if one is ever synthesized) is
        // still READ, not called.
        var asyncMethodSwiftName = NameProvider.EscapeSwiftKeyword(method.Name);
        var callExpr = method.IsExtensionPropertyGetter
            ? $"__self.{asyncMethodSwiftName}"
            : $"__self.{asyncMethodSwiftName}({callArgsString})";
