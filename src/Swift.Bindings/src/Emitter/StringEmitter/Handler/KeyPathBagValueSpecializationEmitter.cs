...
internal static class KeyPathBagValueSpecializationEmitter
{
        ...
        ILogger logger)
    {
        ...
        foreach (var (method, shape) in eligible)
        {
            ...
            foreach (var conformer in conformers)
            {
                ...
                // A conformer TypeSkipPrePass withdrew stays in the type-decl index; naming it
                // would emit a global::<type> reference with no C# declaration (CS0234). Withdraw
                // it here — the same shared oracle the CSM conformer gates use.
    ...
    private static void EmitSwiftTrampoline(SwiftWriter swiftWriter, StagedOverload ov)
    {
        ...
        swiftWriter.WriteLine($"Swift.fatalError(\"[SwiftBindings] {ov.CdeclSymbol}: KeyPath value type not in expected variant set [{variantList}]\")");
