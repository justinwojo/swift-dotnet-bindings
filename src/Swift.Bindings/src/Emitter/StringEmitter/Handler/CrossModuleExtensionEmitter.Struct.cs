...
public static partial class CrossModuleExtensionEmitter
{
        ...
        SimpleEnumLowering? returnEnumLowering)
    {
        ...
        foreach (var p in parameters)
        {
            ...
            {
                swiftWriter.WriteLine($"guard let {p.SwiftBindingName}Val = {p.SimpleEnumQualifiedSwiftType}(rawValue: {p.SwiftBindingName}) else {{ Swift.preconditionFailure(\"[SwiftBindings] Invalid raw value \\({p.SwiftBindingName}) for {p.SimpleEnumQualifiedSwiftType}\") }}");
            }
        }
        ...
        SimpleEnumLowering? enumLowering)
    {
        ...
        {
            swiftWriter.WriteLine($"guard let newValueVal = {el.QualifiedSwiftType}(rawValue: newValue) else {{ Swift.preconditionFailure(\"[SwiftBindings] Invalid raw value \\(newValue) for {el.QualifiedSwiftType}\") }}");
