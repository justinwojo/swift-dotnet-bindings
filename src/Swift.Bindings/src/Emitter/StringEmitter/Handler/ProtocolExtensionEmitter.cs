...
public static class ProtocolExtensionEmitter
{
    ...
    /// <summary>
    /// Renders a parsed argument's Swift EXTERNAL label the way a swiftinterface PrintedName
    /// does. A Swift wildcard label (<c>_</c>) reaches the emitter as the parser's synthesized
    /// <c>arg{i}</c> placeholder — that is a C# identifier, never a Swift label, so rendering it
    /// verbatim produces a key (<c>apply(arg1:context:)</c>) that can never match the
    /// swiftinterface-derived <c>apply(_:context:)</c>.
    /// </summary>
    internal static string GetExternalLabel(ArgumentDecl arg)
    {
    ...
    internal static string BuildMethodKey(MethodDecl method)
    {
        ...
        var labels = string.Join("", method.CSSignature.Skip(1).Select(arg => $"{GetExternalLabel(arg)}:"));
    ...
    internal static string BuildOverloadAwareMethodKey(MethodDecl method)
    {
        ...
        var labelTypes = string.Join(",", method.CSSignature.Skip(1).Select(arg =>
        {
            ...
            return $"{GetExternalLabel(arg)}:{typeName}";
        }));
