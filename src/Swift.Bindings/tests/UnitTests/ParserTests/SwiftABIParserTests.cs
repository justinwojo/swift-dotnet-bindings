...
public class SwiftABIParserTests
{
    ...
    // NOTE: Finding 17 removed SwiftABIParser.DetectAsyncFromMangledName (the "Ya" substring scan).
    // Async detection now walks the demangled node tree via Swift5Demangler.HasAsyncMarker, covered
    // by DemanglerTests/AsyncMarkerTests.cs (including the former false-positive-resistance cases).
