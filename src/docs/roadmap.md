...
| Item | Notes |
|------|-------|
| **Closure-context owner token (Bug 1, 0.10.0 deferral)** | Bridges every escaping-closure GCHandle leak Cat 2/3/4. Restrict the box owner-token model to escaping closures only, mark the Swift box `@unchecked Sendable`, link the destroy hook against `libSwiftBindingsRuntime.dylib` (no `-undefined dynamic_lookup`), and have the adapter closure body explicitly capture the box object. See [`bug-0.10.0-callback-trampoline-gchandle-leak.md`](bug-0.10.0-callback-trampoline-gchandle-leak.md) §"0.10.0 status" for the 4 issues Codex flagged in the prior attempt. Re-enable `LifetimeTrackingTests.TestEphemeralClosureReleasesCapturedSafeHandle` when shipped. |
