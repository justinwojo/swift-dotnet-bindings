...
**P6 — Auto-inject cross-framework dep edges for apple-framework mode.** Surfaced during Session 7 packaging-contract Codex review. Today the SDK only auto-detects cross-module dependencies in `--xcframework` mode (`BindingsGeneratorCommand.cs:452` gates `BinaryDependencyAnalyzer.Analyze` on `hasXcframework`); `<SwiftAppleFrameworkTarget>` projects must declare a manual `<PackageReference Include="SwiftBindings.Apple.<DepModule>" Version="…" />` or the packed nupkg ships without the inter-package dep edge in its nuspec. RealityKit/RealityFoundation hits this directly — see `src/docs/realitykit-shipping-plan.md::Packaging contract` for the manual workaround. Fix direction (sketch — needs design pass): detect cross-module imports from the apple-framework input (via the framework's `.tbd` re-export list or a `swift-api-digester` import dump emitted alongside the existing ABI dump), then synthesize `<PackageReference>` items in the apple-framework path keyed by a known `module → SwiftBindings.Apple.<Module>` map plus the current per-train range. Reusing the xcframework-path `_ResolveSwiftAutoDetectedDependencies` resolver (`Sdk.targets:939–1006`) is *not* a drop-in: it locates sibling binding projects relative to the dependency's `XCFrameworkPath`, but Apple system framework dependencies live inside the Xcode SDK (`_SwiftAppleFrameworkSdkPath`), not sibling apple-framework project dirs — apple-framework auto-injection needs its own resolver that emits a NuGet-only `<PackageReference>` and skips the local-`<ProjectReference>` branch. Authorized but deferred — RealityKit's two-package case is small enough to ship with the manual block; auto-injection becomes load-bearing as more multi-Apple-framework packages land.
