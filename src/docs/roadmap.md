...
| **AppIntents `_IntentValue` projection (prereq for 8b/8c)** | `AppIntents._IntentValue` is the load-bearing constraint across `IntentParameter<Value>`, `EntityProperty<Value>`, `AppShortcutParameterPresentation<…,Value,…>`, plus ~10 sibling PAT-shaped types. It's an underscored SPI protocol currently dropped by parser underscore-suppression, so all dependent types are suppressed with `IndeterminatePwtShape`. Until projected (via a parser-side keep-list entry, then exercised through the existing CSM machinery against the ~8 SDK-shipped `_IntentValue` conformers plus user-declared `AppEntity`s), 8b and 8c cannot ship. Spike result documented in `keypath-subsystem/08c-appshortcut-parameter-presentation.md` Phase 0 result. Comparable to Session 8 v1 ObjC class init work; estimated own session. |
