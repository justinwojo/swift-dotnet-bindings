...
Skip rate is 22.2% (per current `.validation-baseline.json`). The remaining skips are overwhelmingly either correct behavior (private API, synthesized Codable) or architecturally blocked. Consumer-impactful patterns (`Result<T,E>`, common generics, protocol conformances) are already covered. Further reduction has diminishing returns.
...
**~~P2~~ — Wrapper-emission bugs surfaced during ARRaycastQueryTarget Codex review. ✅ Resolved.** Three issues called out by Codex session `019de6ef-c41c-7fb3-a708-cda5cde59cf1`, plus six additional adjacent-path metatype bypasses caught during a follow-up Codex review loop. **Bug 1**: `@available` was not propagating onto `@_silgen_name` / `@_cdecl` Swift wrapper functions in four secondary emission sites — closures (`ClosureEmitter.SwiftWrapper.cs`), large-Optional returns (`OptionalPointerWrapperEmitter.cs`), ObjC-overridden properties (`ObjCOverridePropertyWrapperEmitter.cs`), and ArraySlice normalization (`ArraySliceNormalizationEmitter.cs`). All four now call `WrapperEmitterHelpers.MergeAvailability` + `EmitSwiftAvailability` before the wrapper annotation, mirroring the primary `MethodHandler` / `ProtocolExtensionEmitter` pattern. RealityFoundation now emits 16,218 `@available` lines on its wrapper functions and compiles clean. **Bug 2**: `Optional<AnyClass.Type>` was emitting a bare `Type` token because `MetatypeStrategy` collapses `Optional<Metatype>` to `AnyType` and `WrapperValidation.IsMetatypeType` only recognized bare metatypes. Added a sibling helper `IsMetatypeTypeIncludingOptional` for use only at wrapper-eligibility gates; kept `IsMetatypeType` untouched so `MetatypeStrategy`'s collapse semantics are preserved. Also added `!IsMetatypeType` guards in `CdeclParamMapper.IsProtocolExistentialType` / `IsOptionalProtocolExistential` to prevent metatype-as-AnyType from being misclassified as a protocol existential. Parchment now compiles clean. **Bug 3** (FSPagerView): turned out to be a reincarnation of Bug 1/Bug 2 — passes validation post-fixes with no separate work needed. **Adjacent-path gates** added during follow-up review (each path was previously bypassing the metatype gate): primary method-wrapper gate (`MethodWrapperEmitter.ShouldEmitWrapper` param + return), shared function-shape gate (`WrapperValidation.HasCdeclCompatibleFunctionShape` Guard 10b for return), constructor wrapper (`ConstructorWrapperEmitter.ShouldEmitWrapper` params), property wrapper (`PropertyWrapperEmitter.ShouldEmitWrapper` value type), ObjC override property wrapper (`ObjCOverridePropertyWrapperEmitter.ShouldEmitWrapper` value type), subscript wrapper (`SubscriptWrapperEmitter.ShouldEmitWrapper` return + index params), enum case factory (`EnumCaseWrapperEmitter.IsSupportedAssociatedValueType`), method-generic bridge (`MethodGenericBridgeEmitter.TryEmit` / `IsEligible` return), async per-param eligibility predicate (`MethodHandler.cs` async path), closure swift wrapper (`ClosureEmitter.SwiftWrapper.cs` `CanConvertToCdecl`), large-Optional pointer wrapper (`OptionalPointerWrapperEmitter.CanConvertToCdecl`), and ArraySlice normalization wrapper (`ArraySliceNormalizationEmitter.CanConvertToCdecl`). Telemetry/rejection-reason routes were updated in lockstep (`WrapperValidation.GetRejectionReason`, `MemberValidationPipeline.GetConstructorWrapperRejectionReason`, `SubscriptWrapperEmitter.GetRejectionReason`). **Validator-gate retightening**: confirmed no-op — the CS0535 fix that would have loosened `MemberEmissionValidator.cs`/`MethodHandler.cs` was abandoned in `c614ae45` (favored data-side `apple-frameworks.json` route), so there is nothing to retighten on main. **Unit pinning**: `IsMetatypeType_OptionalMetatype_ReturnsFalse`, `IsMetatypeTypeIncludingOptional_*` (3 cases), `ShouldEmitPropertyWrapper_OptionalMetatypeProperty_ReturnsFalse`, `ShouldEmitWrapper_OptionalMetatypeParameter_ReturnsFalse`, `ShouldEmitWrapper_OptionalMetatypeReturn_ReturnsFalse`, `GetRejectionReason_OptionalMetatypeParam_ReturnsMetatypeParam`, `GetRejectionReason_OptionalMetatypeReturn_ReturnsMetatypeReturn`, `ConstructorWrapper_OptionalMetatypeParam_Rejected`, `SubscriptWrapper_OptionalMetatypeReturn_Rejected`, `SubscriptWrapper_OptionalMetatypeIndexParam_Rejected`, `ShouldEmit_OptionalMetatypeAssociatedValue_ReturnsFalse`, `ShouldEmit_BareMetatypeAssociatedValue_ReturnsFalse`, `IsEligible_OptionalMetatypeReturn_ReturnsFalse` (MethodGenericBridgeEmitter), `ShouldEmitWrapper_OptionalMetatypeProperty_ReturnsFalse` (ObjCOverridePropertyWrapperEmitter). **End-to-end pinning**: full validation across 129 libraries baseline-clean with the four originally-affected libraries (RealityFoundation/RealityKit/Parchment/FSPagerView) confirmed individually.
...
**~~P5~~ — Wire `Swift.Runtime` trimmer descriptor for downstream NativeAOT consumers via `buildTransitive`. ✅** `src/Swift.Runtime/src/ILLink.Descriptors.xml` is the single source of truth, consumed three ways: (1) `<EmbeddedResource>` in `Swift.Runtime.csproj` for the IL trimmer's auto-discovery path (trimmed-but-not-AOT consumers), (2) packed at `buildTransitive/ILLink.Descriptors.xml` next to `buildTransitive/SwiftBindings.Runtime.targets`, which injects `<TrimmerRootDescriptor>` + `<IlcArg Include="--descriptor:..." />` when `PublishAot=true` is detected (NuGet PackageReference flow), and (3) referenced directly via path from `BindingTests/RuntimeTestsApp/RuntimeTestsApp.csproj` (since `buildTransitive` doesn't flow over `ProjectReference`) — replacing the previously-duplicated Swift.Runtime + ValueTuple blocks in `TrimmerRoots.xml`. PackGate (`build/Build.PackGate.cs`) gained two new gates: `AssertRuntimeBuildTransitiveLayout` confirms both files ship in the nupkg AND enumerates every TFM-specific `lib/<tfm>/Swift.Runtime.dll` to verify the embedded `ILLink.Descriptors.xml` manifest resource (PEReader + MetadataReader, hermetic); `AssertRuntimeAotDescriptorInjection` spins up a tiny `net10.0` consumer with `PublishAot=true` and runs `dotnet msbuild -t:DumpInjectedItems` to verify the targets actually inject `IlcArg` + `TrimmerRootDescriptor` items. Behavioral end-to-end coverage stays on `RuntimeTestsApp` (physical iPhone, NativeAOT) — device pass count went 1784 → 1787 after the cleanup. Downstream NuGet consumers (RealityKit/RealityFoundation) now receive the trimmer roots automatically; no hand-rolled descriptor required.
