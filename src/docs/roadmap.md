...
| Item | Notes |
|------|-------|
| **Existential-resolver cross-module USR-to-printedName reconciliation** | Parser sees module via printedName, TypeRecord via USR. Today this is covered by per-framework data fixes in `apple-frameworks.json` (e.g. ARRaycastQueryTarget). A code-side fix would replace those data entries and generalize to AVFoundation and other frameworks. The data fix is honest, not a hack — un-block when a non-data-fixable case surfaces. |
| **Enum TypeRecord synthesis for ObjC enum USRs** | Synthesize Enum-kind TypeRecord (not Class) for nested ObjC enum `c:@E@…` USRs in autoBridge modules. Companion to the existential-resolver item above; currently absorbed by the same data-side fix. Authorized; deferred while data fix suffices. |
