...
**The gates are directional: they ask whether what we emit compiles, never whether we stopped emitting.** `nuke validate` structurally cannot catch surface shrink (a cell with fewer members still compiles); `--skip-surface` logs a vanished skip marker as an improvement; the API-manifest ratchet is additive-tolerant by design and reports removals without failing. Corollary: **a program that trades surface for compilability needs a surface meter, and the meter has to exist before the program starts.** Related, from the same wave's pure fixture-coverage misses: a fixture accompanying a change to *which shapes reach an emission path* must enumerate parent kinds (class/struct/enum × member/ctor), member asyncness/wrapper mode, and module locality (local/cross-module) — one shape proves the path, not the reach.
