...
| **Nested-type-on-generic-outer arg mis-placement** | Surfaced 2026-04-23 by Round 5 Session 1 (E.1). When a generic enum's payload tuple element is a nested type of the same outer (e.g. `VerificationResult<T>.unverified((T, VerificationResult<T>.VerificationError))`), the outer's generic args mis-bind onto the inner nested type, producing `VerificationResult.VerificationError<Swift.AnyType>` instead of `VerificationResult<TSignedType>.VerificationError`. Causes `EnumHandler.CaseConstruction` to bail on the case, leaving `TryGetUnverified` unemitted on `StoreKit2.VerificationResult<T>`. Single-payload `TryGetVerified` is unaffected and ships. Root cause: `GetCSharpTypeNameForEnumCase` (`EnumHandler.CaseConstruction.cs:749`) routes through `BoundGenericsHandler.TranslateBoundGenericTypeToCSharp` with `GenericContext.Empty`, and `TranslateBoundGenericTypeToCSharp` (`BoundGenericsHandler.cs:649–724`) appends the outer's generic args to the end of the resolved nested FQN (wrong segment) while resolving each param via the empty context → `AnyType`. `QualifyNestedGenericOwners` (line 876) exists to fix segment placement but no-ops when `GenericContext` is empty and `moduleDecl` is null, which is the enum handler's call. Fix requires (a) segment-aware generic-arg placement, (b) resolving outer params via the enum's own generic params instead of empty context, (c) handling the redundant-C#-generic pattern the emitter uses for nested types under generic outers (currently triggers CS0693 + CS0305 in any consumer with `TreatWarningsAsErrors=True`; passes `nuke validate` only because the validation csproj in `/tmp/binding-validation-<branch>/` does not inherit `Directory.Build.props`). Runtime verification for the tuple-with-nested-type factory-thunk marshalling path needed once the emission compiles. |
