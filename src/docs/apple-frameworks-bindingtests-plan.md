...
7. **MusicKit smoke test.** ✅ **COMPLETE** — commit `49bea2a1`. `MusicItemID` string round-trip through the direct-mode pipeline + reflection-only pin on `Song.AudioVariants` (ios15 type / ios16 property) for fix #2 per-property `@available` propagation. No `MusicAuthorization.request()`, no `MusicCatalog` network fetch, no authorization sheet.
8. **tvOS simulator runner + Catalyst resolver unit test.** ⚠️ **PARTIAL** — commit `e88fef77` shipped the full runner plumbing (csproj, Nuke target, device-family abstraction, parameterized SimCtl, platform-stamp sidecar) and all 3 Catalyst resolver unit tests. **tvOS end-to-end gate deferred**: on the build host, (a) `SwiftBindingsTestLibDependency` regen exits non-zero under `--platform tvos` because the dep module has iOS-only APIs that don't compile for the tvOS slice, and (b) `RunBuildBridge()` is iOS-hardcoded and fails with `no such module 'SwiftBindingsTestLib'` against a tvOS-only slice. Both are real follow-ups — see new Session **8.5** below. All code paths up to and including `dotnet build` of the tvOS app bundle are verified. To run the e2e gate locally, first install the runtime: `xcodebuild -downloadPlatform tvOS`. Session 8 also fixed 5 AI-review findings before commit (crash-log prefix-match parameterization, `SimCtl.Launch` appName requirement, platform-stamp sidecar mirroring smoke-flags staleness on the platform axis, `Program.cs` `effectiveArgs` plumbing, tvOS UDID family validation). Includes the original deliverable: new `RuntimeTestsApp.tvOS.csproj`
8.5. **tvOS end-to-end unblock (NEW, re-scoped from Session 8).** Two halves, both real generator/build-side work beyond the Session 8 scope:
    - **(a) `SwiftBindingsTestLibDependency` tvOS compile.** The dep module's Swift source references iOS-only APIs (identify which ones via `nuke build-xcframework --platform tvos` failure output). Either guard the iOS-only usages with `#if os(iOS)`, or split the iOS-only shape into a separate fixture that's excluded from the tvOS xcframework build. Pick after inspecting what's actually referenced.
    - **(b) `RunBuildBridge()` tvOS support.** The bridge build currently hardcodes iOS paths/triples. Parameterize it on `ApplePlatform` the same way Session 8 parameterized `RunRegenerateBindings` and the SimCtl helpers. After both (a) and (b) land, `nuke runtime-tests-tvos-simulator` should be green end-to-end (runtime must be installed via `xcodebuild -downloadPlatform tvOS` first).
    - Blocks: tvOS smoke-test wiring (per-framework opt-ins would depend on a working tvOS runner).
