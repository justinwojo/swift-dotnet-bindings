...
| MusicKit | None at runtime. | `MusicCatalogResource` enum-case `@available` paths. Most MusicKit APIs need entitlements; pick metadata-only constructors. |
...
| `MusicKitSmokeTests.cs` | `MUSICKIT_SMOKE` | *Candidates:* decode a known `MusicCatalogResource`-shape JSON payload via the framework's `Decodable` conformance. **Avoid anything that hits MusicKit's authorization sheet.** Confirm against the snapshot. | Enum-case `@available` paths (fix #2) on a Tier-A framework. |
