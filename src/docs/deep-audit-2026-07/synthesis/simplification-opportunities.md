...
| **S1-37** | **[Verification 2026-07-16: REFUTED — §4.3] ~~Obsolete `ComputeEntryPoint(MethodDecl)` pre-AF13~~** — NOT dead: 3 live production callers (`CrossModuleExtensionEmitter.cs:633, 694, 734`), and the overloads genuinely diverge post-AF13 (decl-only returns the unpromoted mangled name). The "external tooling" risk named here is the wrong risk entirely. Do NOT delete; superseded by **D7** (migrate the 3 call sites to the env overload, then retire). | Entry-point helpers | ~~low / byte-identical~~ **not unused** | **No** | ~~External tooling~~ 3 live callers; see §4.3 | A1-C01 |
