...
| 8a-2 — pre-8b structural gaps: `_IntentValue` conformance-record ingestion (Gap A) + decoupling synthesized public-underscore protocol names from the Pattern-2 internal-reach set (Gap B) | shipped — `UnderscoreProtocolSynthesizer.IngestStrippedConformances` re-attaches digester-stripped conformances (empty descriptor, type-database fact only); `MergeSuppressedIntoInternalTypeNames` keeps synthesized names out of `InternalTypeNames`. Regen: Pattern2InternalTypeReach 784 → 0, `_IntentValue` suppressions 17 → 12. | uncommitted in `keypath-worktree` |
| 8a-3 — constructed-generic constraint silent-drop (Gap C) | shipped — `GenericSignatureParser.ParseConstraint` returns null for constructed-generic targets (`KeyPath<Intent, Parameter>`) instead of throwing through `SwiftABIParser.HandleNode` and discarding the enclosing decl; all five `AppShortcutParameterPresentation*` structs now emit. | uncommitted in `keypath-worktree` |
| 8b — `EntityProperty.init<Entity>(…)` KeyPath-keyed convenience-init family | shipped — `ConformerKeyPathInitFactoryEmitter` emits consumer-side factory methods constructing a dependency's `EntityProperty<Value>` via a closed-`AppEntity` Swift trampoline over `init<Entity>(…getter:|getSetter:)`, consuming `{Conformer}AppEntityKeyPaths` singletons. PublicType/MarshalFromSwiftType split fixes string-value type-metadata; recognizer admits only single-protocol method-own-generic shapes (RWKP + multi-constraint rejected). 17 unit + 8 BindingTests; sim 2277, device 2298. Real AppIntents `EntityProperty` convenience-init overloads remain blocked (documented in `08b-…md`). | uncommitted in `keypath-worktree` |
| 8c — `AppShortcutParameterPresentation` higher-kinded `ParameterKeyPath` | actionable — 8a-2 + 8a-3 prerequisites shipped (the five ASPP structs now emit; four-generic constraint shape compiles, spike at `/tmp/8c-spike/`); not yet started. Note the primitive-`_IntentValue`-conformer caveat in `08c-…md`. | — |
| 8d — `PartialKeyPath<CSSearchableItemAttributeSet>` `indexingKey:` parameter | **parked as v2** — preflight surfaced a structural blocker: `CSSearchableItemAttributeSet` is ObjC-rooted (`declKind=Import`) and not bound by the Swift binding pipeline. Framework `wrapperImportable: true` flip + unit-test row landed (`d408df92`); typed-singleton container deferred until ObjC-rooted projection is in scope. See `08d-…md` "Phase 8d.1 preflight result". | partial: `d408df92` |
