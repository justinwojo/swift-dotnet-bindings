...
Shipped / superseded session docs (1–5, 6b, 6c, 7, 8 and its 8b/8c/8d/followups, 11–14) have been moved to `Done/` to declutter the active set; their filenames are unchanged. The only docs in the folder root are the live ones: `00-overview` (this tracker), `06` (carries the `MusicLibrarySectionedRequest` follow-up), `08-remaining` (single source of truth for all remaining Phase 8 AppIntents/CoreSpotlight KeyPath work), and `09`/`10` (not started). **For remaining Phase 8 work, read `08-remaining.md`, not the archived 8-series docs.**
...
- **SwiftUI view-tree / `ViewBuilder` result builders** — KeyPath bindings would work with SwiftUI in isolation, but the result-builder DSL has no ABI representation, so there is no C# caller that can compose the view tree a `Binding`/`ObservedObject` projection feeds. This is why Session 9 is *declined*, not deferred (see Status). The supported SwiftUI path is the hosting bridge (`SwiftUIBridgeEmitter`), documented in `Design/apple-framework-portfolio.md`.
...
Sessions 1–8, 11, 12, 13, and 14 have shipped on `keypath-subsystem` / `keypath-worktree`. Session 8 v1 was deliberately scoped down once the real `AppIntents.swiftinterface` was inspected — the original plan described API shapes (`EntityProperty<Entity, Value>.property(getter:)`, `AppShortcutParameterPresentation<Entity>.keywordTitle(_:)`) that don't exist in the SDK. The real KeyPath-keyed surface lives in three follow-ups (8b/8c/8d), each Session 4-scale machinery on its own. Sessions 13 + 14 closed the SB0001 generic-host wrapper gap (parent-generic constructors, KeyPath family, Array<T>, nested-of-parent including cross-host fallback, method-own-generic constructors via phantom-box GSF, variadic-pack splat, and the non-final-generic-class `init()` residual). The shared 8b/8c prerequisite — `UnderscoreProtocolSynthesizer` for `_IntentValue` / `_ParameterSummarySwitchCase` — shipped and projects `EntityProperty<Value>` / `IntentParameter<Value>` as C# classes; the pre-8b structural gaps (conformance-record ingestion, synthesized-name internal-reach cascade) and the 8c silent-ASPP-drop gap closed as 8a-2 + 8a-3 (`510d8717` + `c5698e43`). **The entire 8-series is now committed and archived to `Done/`.** 8c v1 shipped (the higher-kinded `ParameterKeyPath` capability + primitive-conformer relaxation, `a1152f1a`); its per-tuple `AppShortcutParameterPresentation` emission was *declined*, not deferred — an upstream framework-design blocker (no C#-constructible sink). 8b's KeyPath singletons + consumer-side factory shipped (`cb07dfe3`); the real AppIntents `EntityProperty` convenience inits remain blocked. 8d is parked as v2 (`CSSearchableItemAttributeSet` is ObjC-rooted). **All remaining Phase 8 work now lives in the single source-of-truth doc `08-remaining.md`** (R1 real-`EntityProperty` inits, R2 CoreSpotlight container, R3 cross-module conformer enumeration, R4 shared-core extraction, R5 latent value-type availability).

**Sessions 9 (SwiftUI) and 10 (residual consumers) are declined**, not deferred. Their planned *KeyPath-binding* surface is unreachable from a .NET consumer: SwiftUI view composition is blocked by the `@ViewBuilder` / `@resultBuilder` wall (a compile-time Swift feature with no ABI representation — `roadmap.md:148,150`), so even a fully-bound `Binding` / `ObservedObject` `@dynamicMemberLookup` (the one place `ReferenceWritableKeyPath` would matter) has no C# caller that can assemble the view tree it projects into. Session 10's residuals are portfolio hard-skips for the same or adjacent reasons — Charts / SwiftData / Observation are macro-driven declarative DSLs with no runtime surface, and UIKit duplicates `dotnet/macios`. SwiftUI's real, useful .NET story is the auto-generated **hosting bridge** (`SwiftUIBridgeEmitter`), which wraps any concrete `View` in a `UIHostingController` and now exposes it as a typed `UIKit.UIViewController` (the `ViewController` accessor, alongside the raw `GetViewController()` IntPtr) ready to embed in a UIKit / .NET MAUI hierarchy. "Which Apple frameworks are worth binding" is no longer tracked in this subsystem — that decision framework lives in [`Design/apple-framework-portfolio.md`](../Design/apple-framework-portfolio.md), whose *SwiftUI bridging capability* section is the authoritative account of what the bridge does and does not handle.
...
| 8a-2 / 8a-3 — pre-8b structural gaps (`_IntentValue` conformance-record ingestion, synthesized-name internal-reach decoupling, constructed-generic constraint silent-drop) | shipped — `UnderscoreProtocolSynthesizer.IngestStrippedConformances` + `MergeSuppressedIntoInternalTypeNames`; `GenericSignatureParser.ParseConstraint` returns null on constructed-generic targets instead of discarding the enclosing decl (all five `AppShortcutParameterPresentation*` structs now emit). | `510d8717` + `c5698e43` |
| 8b — `EntityProperty.init<Entity>(…)` KeyPath-keyed convenience-init family | shipped (singleton + factory half): `AppEntityKeyPathSingletonEmitter` + `ConformerKeyPathInitFactoryEmitter` against the `MiniEntityProperty` stand-in. **Real AppIntents `EntityProperty` inits remain blocked → `08-remaining.md` R1.** | `cb07dfe3` |
| 8c — `AppShortcutParameterPresentation` higher-kinded `ParameterKeyPath` | v1 shipped (higher-kinded constraint capability + Phase A/B primitive-`_IntentValue`-conformer relaxation). Per-tuple emission **declined** (upstream framework-design blocker, no C#-constructible sink — see `Done/08c-…md` "Phase C feasibility audit"). No remaining actionable 8c work. | `a1152f1a` + `c5698e43` |
| 8d — `PartialKeyPath<CSSearchableItemAttributeSet>` `indexingKey:` parameter | **parked as v2** — `CSSearchableItemAttributeSet` is ObjC-rooted (`declKind=Import`), not bound by the Swift pipeline. `wrapperImportable` flip + unit-test row landed; typed-singleton container blocked → `08-remaining.md` R2. | partial: `d408df92` |
| 9 — SwiftUI + SwiftUICore productionization | **declined** — KeyPath-binding surface unreachable from C# (`@ViewBuilder`/result-builder wall, `roadmap.md:148,150`); SwiftUI's real story is the hosting bridge, now with a typed `UIViewController` accessor. See Status prose + `Design/apple-framework-portfolio.md`. | — |
| 10 — Residual consumers cleanup (Charts + SwiftData + Combine + UIKit + Observation; A-1 closure) | **declined** — Charts/SwiftData/Observation are macro/DSL portfolio hard-skips; UIKit duplicates `dotnet/macios`. Framework-selection decisions moved to `Design/apple-framework-portfolio.md`. | — |
| 11 — CSM defensive conformer-constraint filter + rejection tombstones | shipped | `5e46c711` |
| 12 — AppIntents 0.12 platform parity (iOS `_const` init filter; macOS schema/availability cascade; KeyPath singleton emitter consolidation; `EntityQuerySort.By` residual) | shipped | `d408df92` + `510d8717` + `77f19a80` + `874936a8` + `1a623f26` |
| 13 — SB0001 generic-host wrapper gap (parent-generic constructors / instance generics) | shipped — KeyPath-family + Array<T> + nested-of-parent (outer==parent) admitted via GSF; closes 5/9 AppIntents sites + KeyPath constructor crasher. The remaining 4 AppIntents sites span 3 *different* emission mechanisms outside doc 13's parent-generic scope and are continued in doc 14. | `bd4aaa42` |
| 14 — Remaining SB0001 surface after doc 13 (cross-host nested-of-parent held at SB0001 by-design per Hypothesis 3; method-own-generic constructors via phantom-box GSF; variadic-pack splat; return-type-only overload disambiguation; non-final-generic-class `init()`; foreign-type-extension `@available` propagation) | shipped — AppIntents.cs SB0001 grep 7 → 1 (the one remaining is the by-design cross-host case); sim 2254/0, device 2275/0; Codex r1+r2 + Grok r1 clean | `8ead3167` |
