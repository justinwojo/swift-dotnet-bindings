...
Closed-out status as of the doc-sync pass following `62ec673e`.

- **`MusicLibrarySectionedRequest<SectionType, MusicItemType>`**: 0/17 surface members emit (empirical). 64 cartesian `*CsmExtensions` classes are emitted but empty; the 17 surface methods (`filterItems` ×8, `sortItems` ×1, `filterSections` ×7, `sortSections` ×1, `response` ×1) all tombstone with "protocol with associated types used as constraint" — per-method `where SectionType : MusicLibraryRequestable` clauses on a two-PAT-generic-parent type aren't handled by current CSM filter machinery or Route C (`RouteCSortShapeEligibility.cs:72` gates on single-generic-parent). Recorded as an architectural gap in `00-overview.md` Status notes; not promoted to roadmap.md until a consumer asks.
- **Device gate**: green (`nuke binding-tests --device`, pass count 2227 vs 2213 baseline, 0 crashes) after fixing the CSM `FromX()` / generic-param-return cleanup in `ConcreteProtocolSpecializationEmitter.cs:1402-1440`. Three NewFromPayload contracts now discriminated by `TypeRecord` shape: **direct-wrap** (non-frozen struct, complex enum) → `NativeMemory.Alloc` + no `try/finally` (the returned SafeHandle owns the buffer, matches its `NativeMemory.Free`); **copy-out** (`IsFrozenStructProjectedAsClass` = frozen + `RequiresMemoryManagement`) → `AllocHGlobal` + `try/finally{FreeHGlobal}` + `SwiftMarshal.DestroyWireBufferRetains<T>` (NewFromPayload `InitializeWithCopy`'s, so wire still holds `+1` on internal refs that must be released); **pure value** (frozen, no `RequiresMemoryManagement`) → `AllocHGlobal` + `try/finally{FreeHGlobal}` (byte copy, no retain semantics). `returnsGenericParam` paths resolve the conformer's record so non-frozen-struct conformers (e.g. `TaggedSearchItem` through `ValidateAndReturnTagged<T>`) also take the ownership-transfer shape (sim +1, 2201 → 2202). When the conformer record can't be resolved, the discriminator defers to `ClassifyConformerForCSharp` (same source preflight uses to admit the pairing) so frozen-with-memory cross-module conformers don't silently leak through an over-broad direct-wrap default. Two latents remain: `MethodGenericBridgeEmitter.cs` (lines 731/741/834-836/861, same alloc+free antipattern, no test exercises it) and class-conformer `returnsGenericParam` (Swift writes class pointer into carrier via `initializeMemory`, C# `NewFromPayload` wraps the carrier address — pre-existing, no test exercises it). Tracked as follow-up.
- **`00-overview.md` Status section**: refreshed with per-session shipped-at commits + `MusicLibrarySectionedRequest` exclusion note.
- **`06-musiclibraryrequest-re-enablement.md` exit-criteria checklist**: closed in `06-musiclibraryrequest-re-enablement.md` itself (the canonical mirror).
- **A-1 deferral**: no active release doc references it. `src/docs/sdk-0.11.0-remaining.md` does not exist; nothing in `src/docs/` outside `keypath-subsystem/` carries an A-1 deferral. The doc was retired before 0.11.0 shipped. No edit required.
- **Public wiki**: `Known-Limitations.md` (wiki repo `/Users/wojo/Dev/swift-dotnet-bindings.wiki`) contains no "no sort for `MusicLibraryRequest`-style PAT generics" caveat to retract. MusicKit is already documented as a fully-bound framework. No edit required.
- **Future docs**: `git status` clean — both `foundation-nsobject-typed-upgrade.md` and `property-getter-constrained-generic.md` from earlier worktree state have already been resolved (no longer untracked).
