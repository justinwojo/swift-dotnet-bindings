...
**Date**: 2026-04-09
**Grounded in**: StoreKit 2 (the first Apple framework brought up end-to-end through this pipeline; see `0.8.0-storekit2-exploration.md` for the full Sessions 1–7 trail)
**Sibling**: `../0.8.0-plan.md` § Phase 1 (the working assumption in that doc was falsified during exploration; this doc captures what actually worked)
...
These are the twelve questions in `0.8.0-plan.md` § "Strategy questions to answer in Phase 1". Each answer is grounded in something StoreKit 2 actually exercised, with a pointer to the evidence.
...
**Evidence**: Session 1 (`108f4f12`) landed Apple-framework target mode; Sessions 2 + 4 (`eafe252d`, `ab3f25a8`) landed wrapper compilation in direct mode and the in-tree `Swift.Runtime` ProjectReference fallback; Session 7 (this session) landed the publishable pack flow. `0.8.0-storekit2-exploration.md` § "Recommended next session" → § "Tracking" has the full chain.
...
- Generator-emitted csproj uses `LibTfm = "net10.0-ios26.0"` for `buildTransitive/<tfm>/` pack paths to avoid NU1012 (Session 7, this session — `PlatformInfo.LibTfm` field)

**Evidence**: `0.8.0-storekit2-exploration.md` § Session 1 outcome (target mode + opaque param fix), § Session 2 outcome (availability propagation + four C# emit bugs subsequently fixed), § Session 4 outcome (mutating async + ProjectReference fallback), § Session 5 outcome (resolver dyld passthrough), § Session 7 outcome (pack-time `LibTfm`).
...
**Evidence**: `0.8.0-storekit2-exploration.md` § Session 5 outcome ("What the resolver bug actually was" + "Fix"). 15 new resolver unit tests in `SwiftFrameworkResolverTests.cs`.
...
**Evidence**: `unzip -l /tmp/storekit2-session7-fresh/nupkg/StoreKit.Swift.iOS.0.8.0-preview.1.nupkg` produces exactly the layout above. Session 7 outcome block in `0.8.0-storekit2-exploration.md` has the verification.
...
**Evidence**: `0.8.0-storekit2-exploration.md` § Session 6 outcome and `roadmap.md` lines 101–102.

### 9. Macro-based frameworks

**Not exercised by StoreKit 2.** StoreKit 2 has no `@Model` / `@Observable` macros. The strategy here is unchanged from `0.8.0-plan.md`: defer SwiftData / Observation until after the simpler Apple frameworks ship. The `.swiftinterface` ought to contain the expanded form, but verifying that is framework #N work (where N is "first macro-heavy framework we attempt"), not framework #2.

### 10. Testing strategy

**Decided** (mirroring how StoreKit 2 was validated):

- **Unit tests** for any generator changes the framework requires. StoreKit 2 added unit tests for parameter-position opaque lowering, mutating-async wrapping, dyld-path resolver passthrough, `EnableDefaultCompileItems=false`, `LibTfm`-based pack paths, and a few more. New generator changes should ship with new unit tests; that's not optional.
...
- **Smoke tests** (`BindingTests/RuntimeTestsApp/SmokeTests/<Framework>SmokeTests.cs`) gated on a per-framework compile symbol (`STOREKIT_SMOKE`, `WEATHERKIT_SMOKE`, ...) and an MSBuild property (`$(StoreKitSmokeEnabled)`, ...). The csproj wires the conditional `<Reference>` + `<NativeReference>` block; the test class is wrapped in `#if`. Fresh checkouts and CI silently skip the test — no artifacts needed. This is the **zero-regression** wiring pattern; framework #2 should copy it shape-for-shape.
...
**Decided**: iOS first. macOS / Mac Catalyst / tvOS support is in `PlatformInfoFactory` and the emitter, but Sessions 1–7 only exercised iOS. Multi-platform validation is Phase 2 (`0.8.0-plan.md` § "Phase 2 — Multi-Platform Validation"). The framework #2 checklist below treats macOS/Mac Catalyst as a stretch checkpoint — if framework #2 ships iOS-only and macOS lands later, that's fine.
...
**Open**: `LibTfm` for macOS / tvOS / Mac Catalyst is hardcoded in `PlatformInfoFactory` to `net10.0-<platform>26.0` matching the iOS form. This needs verification when a non-iOS Apple framework actually packs — the `26.0` value tracks whatever SwiftBindings.Runtime's `lib/<tfm>/` directory is named at, and bumping the platform version means bumping all four entries together. Filed in the framework #2 checklist as a checkpoint.
...
- **NU1012 / `LibTfm` evolution**: `PlatformInfoFactory.LibTfm` is hardcoded `net10.0-<platform>26.0`. Bumping the workload's default platform version means bumping all four entries together. Could be derived dynamically from the workload at runtime, but the cost of the hardcode is one search-and-replace per workload bump and the dynamic form would add complexity. Leave hardcoded; revisit if it bites.

---

## Cross-references

- `../0.8.0-plan.md` § Phase 1 — the original strategy questions this doc answers
- `../0.8.0-storekit2-exploration.md` — the full Session 1–7 trail this doc is grounded in
...
- `src/Swift.Bindings/src/Configuration/PlatformInfo.cs` — `LibTfm` field doc explaining the NU1012 trap
