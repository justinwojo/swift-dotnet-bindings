...
**Companion docs**:
- `src/docs/architecture-gameplan.md` (deleted; recover via git history) — the 1.0 plan (DONE).
- `src/docs/architecture-gameplan-v2.md` (deleted; recover via git history) — round 1 of post-1.0 (`libswiftDemangle` swap + SwiftSyntax producer, both graduated from this doc).
...
## Graduated to v2 (outcomes recorded)

These two items moved to `src/docs/architecture-gameplan-v2.md` (deleted; recover via git history):

1. **`libswiftDemangle` swap** — v2 Milestone 1. Outcome: **NO-GO** (see `src/docs/Design/demangling-replacement-spike.md`); the managed demangler stays.
2. **SwiftSyntax producer behind `SwiftInterfaceFacts`** — v2 Milestone 2. **DONE** — implemented as `SwiftSyntaxInterfaceFactsProducer`, `InterfaceFactsAggregator`, and `SwiftInterfaceFacts` under `src/Swift.Bindings/src/Parser/` (with unit tests).
...
- **`PipelineContext` / kill static collectors.** `ReportCollector` (`AsyncLocal<ReportSession?>` for full session isolation; see `Reporting/ReportCollector.cs`), `AppleSupplementReferences` (`[ThreadStatic]`), `SwiftUIBridgeCollector` (static). Mechanical refactor; pipeline works today.

- **Pipeline stage abstraction + CLI mode dispatcher.** Replace `BindingsGenerator.GenerateBindings` (~1254 LOC as of 2026-07) and `BindingsGeneratorCommand.Execute` (~1987 LOC as of 2026-07, 50+ early returns) with `IPipelineStage` + `ICommandMode`. Refactor; pipeline works today.
...
- **Projection-only Marshaler.** Promote `IProjectionVisitor<T>` to be the only dispatcher; decompose `ClosureHandler` (2487 LOC as of 2026-07), `BoundGenericsHandler` (1885 LOC as of 2026-07), `ExistentialHandler`. Mechanical decomposition; correctness unchanged.
...
- **Strangle post-emission text rewriters (full).** M3 of the gameplan fixes the top causes at emission time. The full subsystem strangle (single `EmissionFeasibilityProbe` consulting per-slice ABI / suppressed proxy refs / etc., retiring `SwiftWrapperPostProcessor`, `SimulatorOnlyMemberDetector`) is post-1.0.
...
- **Build / SDK decompositions.** As of 2026-07: `Build.RuntimeTests.cs` 3462 LOC, `Build.Validation.cs` 2546 LOC, `Sdk.targets` 3820 LOC, `SwiftWrapperCompiler.cs` 3097 LOC. Decompose into strategy interfaces. Mechanical; works today.
...
- **`ExistentialContainer0..8` consolidation.** `ExistentialContainer.cs` 1699 LOC as of 2026-07 (9 copy-pasted structs). Real ABI cleanup; works today.
...
- A runtime regression turns out to be drift between interface-facts producers and a Swift compiler change (SwiftSyntax producer already shipped; revisit producer coverage/facts, not "adopt SwiftSyntax").
