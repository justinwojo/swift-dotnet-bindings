...
## Twelve generic-parent members stay behind fail-closed metadata gates — dynamic witness-table threading and buffer-mode unpacking

**Revisit when:** a consumer needs one of these shapes, or a fixture exists for the dynamic-witness or buffer-mode path on device — open the gate per shape behind its own test, never as a class.

<details>
<summary>Recorded evidence and disposition</summary>

The generic-parent reroute opened generic static dispatch with metadata passed as a parameter, one wrapper per generic *declaration* rather than per specialization. Twelve members did not follow: six need a dynamic protocol-witness-table threaded into the metadata accessor, six need buffer-mode unpacking. Both gates are fail-closed today and each member keeps the direct `CallConvSwift` route with an untyped `SwiftSelf`. **Why they were not opened:** the failure mode is an arm64e pointer-authentication trap from an arity disagreement between the C# P/Invoke, the metadata helper and the real metadata accessor — a runtime trap on a physical device, not a compile error — so each newly opened shape has to ship its own end-to-end test on the metadata path and be verified on device before its gate is relaxed. That is per-shape work, not one change. Three further generic-parent members were left closed for the same reason: an Optional-ABI tag probe, a bridged-URL-collection getter and a static member behind an availability gate. **Trigger:** a consumer needs one of these shapes, or a fixture exists for the dynamic-witness or buffer-mode path on device — open the gate per shape behind its own test, never as a class.
