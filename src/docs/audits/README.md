...
- [x] `Track-A7_Async-Throws-Error-Carrier.md` — **done** (**5/5 Critical**; 10 confirmed: 6 P0 = managed exceptions escaping `[UnmanagedCallersOnly]` async success/error callbacks into native Swift → SIGABRT + hung Task + leak, across `AsyncHarnessEmitter` (primitive/tuple/String/complex) and `AsyncMethodGenericBridgeEmitter` (class-bound generic); only `Array<String>` was hardened; 4 P1 = GCHandle-integer task-identity recycling (stale unregister evicts a live call), per-call async-closure GCHandle leak, typed-throws error rethrow, diverged dead-duplicate emitter cluster; 1 inconclusive double-resume/UAF (reachability-gated); 36 deferred — same crash/leak/identity classes at more sites)
- [x] `Track-A8_Parser-Demangler-Fidelity.md` — **done** (6 confirmed, all P1; risk 3/5; 2 live: demangler `Y`-modifier gap drops `Yb`@Sendable/`YK` typed-throws → no `FunctionReduction` (pervasive in modern concurrency), and `HasUnmatchedOpenParen` string-blind paren count swallows to EOF on a `="("` default; `where O: AnyObject` throws → whole decl silently dropped (live, `<U:AnyObject>` idiom). 3 latent Model-corruption: typed-throws first-match closure-param shadow, public protocol requirements mis-flagged `IsModuleInternal` on both producers. Most defects dormant now but go live as Swift concurrency/value-generics land. 2 clean refutations (`nonmutating func` unreachable — 0 hits in ~3000 SDK interfaces; `indirect enum`/`distributed actor` match fine). 4 inconclusive (operator/`consuming` mis-flag — real but no current reader), 36 deferred)
...
- [x] `Track-C2_Invariant-Drift-Dedup.md` — **done** (5 confirmed, all P1; risk 4/5; one root cause: emitted C# name applies property-collision rename (`Foo`→`FooMethod`) + numeric suffix (`Foo2`) that the dedup keys (`IHandler.cs:528`, `IEnvironment.cs:149`) and the same-module override verifier (`WrapperEmitter.Signature.cs:570`) never see → genuine collisions slip dedup (`CS0111`, "100% coverage" reported) and overrides silently bind to the wrong slot; + stale `constraints.md:18` WasEmitted trap (doc 13/6 vs live 23/12). 6 refuted (probe-tested), 24 deferred — same `propertyNames`-omission family)
- [x] `Track-M1_SwiftUI-Bridge.md` — **done** (5 confirmed: 2 P0 + 3 P1; risk 4/5; the bridge concentrates the codebase-wide crash classes — P0 ObjC-bridgeable struct init param type confusion (C# object pointer read as raw struct bytes via `assumingMemoryBound`, no `IsObjCBridgeable` branch; URL→SIGSEGV; 5 sites), P0 BoundEnum `Type(rawValue:)!` force-unwrap across 6 sites (`(MyEnum)999` from C# → hard trap, externally triggerable); P1 ResultClosure ObjC-bridgeable struct UAF (`Data→NSData` 3/3 crash), P1 frozen-with-ref-fields closure-arg buffer+ARC leak (collapses adopt-vs-copy ownership). **6 refuted with physical-mechanism probes** (BridgeArray Count 4/8-byte SIGSEGV impossible — register zero-extends; resilient `.pointee` uses VWT; `takeUnretainedValue` real +1; 25k-interleaving async monitor = 0 UAF). 1 inconclusive (auto-infer leaf-name coupling, load-bearing but currently correct), 26 deferred — async/TypedClosure ObjC-bridgeable twins unprobed)
- [x] `Track-M3_TypeDatabase-Projection-Parity.md` — **done** (7 confirmed, all P1; risk 4/5; data-table/classification layer — a type's classification disagrees with its real ABI → wrong marshalling: CoreNFC `objcPrefixes:"NK"` (classes are `NFC*`) drops `Optional<NFCTag>`; `UIKeyboardType`/`AVCaptureVideoOrientation` miss `rawValueType` → 4-byte int vs 8-byte Int (high word dropped); 6 optionalFallback modules (CM/CT/CS/PDF/WC) prefixes absent → members dropped/opaque; NSString-typedef structs registered blittable `kind="struct"` → no-ARC pass-through UAF (QuartzCore/AVFoundation/NaturalLanguage, siblings correctly `kind="class"`); `Foundation.Date` tuple element double-vs-DateProjection → CS0029 on return; `typeRemaps` forces ObjC-class on 16-byte value struct `_LocationEssentials.CLLocationCoordinate2D`. 0 inconclusive, 5 refuted with mechanism probes (Vision.VNConfidence input never produced; `Optional<ObjCRooted>` IntPtr ABI correct via SIL nil-ptr-opt; Foundation.Data short-circuit dominates), 33 deferred. Every confirmed item has zero unit coverage)
- [x] `Track-M4_BindingTests-Coverage-Matrix.md` — **done** (8 confirmed: 1 P0 + 7 P1; risk 4/5; the end-to-end gate's skip taxonomy is unreliable — P0 `AsyncGenericContainer.ProcessAsync`/`FetchOrThrowAsync` ship as live non-`[Obsolete]` wrong-ABI public methods (SIGSEGV; skip reason factually wrong, symbol DOES export); multiple `[SkipOnSimulator]` skips misattribute our own CallConvCdecl crashes to "upstream Issue 1" (live sim/device runs PASS) — directly implicates `feedback_mono_jit_blame.md`; macOS/Catalyst gates skip Mono-only tests on CoreCLR (~14); stale skips hide working `TryCreate` behind 5 vacuous bodies; returned-throwing-closure dead cdecl thunk falls back to crashing CallConvSwift invoker; coverage matrix can't detect missing runtime coverage (derives "passing" from source existence, never reads test files). 4 inconclusive (enum raw-value tag-vs-`init(rawValue:)` split), 0 refuted, 45 deferred)
- [x] `Track-M2_Wrapper-SDK-Packaging.md` — **done** (11 confirmed: 1 P0 + 10 P1; risk 5/5; collapses to two root causes — (a) the SwiftFramework/SDK SwiftUI-bridge compile path never threads `--target-architectures`/`WithArchitecture`, (b) the Apple-direct standalone branch hard-codes the bridge "off" after compiling it. P0 `CSharpWrapperCoGater.cs:51` — co-gater matches only `[LibraryImport]`+`partial`, so `[DllImport]`+`static extern` wrapper P/Invokes from 4 KeyPath/AppEntity/enum-metadata emitters survive a stripped `@_cdecl` → dangling P/Invoke (proven via real `Process` call). Bridge-arch cluster (P1×6, one defect across layers `Program.cs:1225`/`SwiftWrapperCompiler.cs:987`/`Sdk.targets:2153`): bridge sim slice stays **arm64-only** while wrapper is fat → DllNotFound on iossimulator-x64/Rosetta; standalone direct-mode also drops the bridge from pack + consumer NativeReference (`BindingsGeneratorCommand.cs:1316`/`:1044`) despite compiling it. P1 `Sdk.targets:2441` SWIFTBIND031 Guard 2a trusts static slice-ids (resync is AppleFramework-only) → false hard-error on x86_64-only/renamed sim slice. P1 `WrapperXCFrameworkMerger.cs:116` non-transactional fat-fold → fat binary advertised single-arch on mid-loop throw (the `Program.cs:943` "fails before overwriting" comment is materially false). 0 inconclusive, 1 refuted (multi-shared-slice topology never produced — but single-slice/ditto-failure window still confirmed), 25 deferred)
- [ ] (Tier-3 reports as scheduled — L1/L2/L3 not run)
- [x] `STATE-OF-THE-CODEBASE.md` — **done** (capstone synthesis of all 14 tracks, 228 lines; ~104 confirmed (~22 P0, ~70 P1), 7 root-cause clusters, single deduped P0→P2 backlog, top-20 files-to-touch-with-care, ~280+ deferred surface noted; floor counts at ~40–60% per-run recall)
...
Inspected: full src/docs/ (roadmap.md + Design/*.md + Future/* upstream issues), .claude/rules/constraints.md (40+ "trap" invariants), AGENTS.md/Claude.md + scoped rules, build/baselines/validation-baseline.json, BindingTests/ structure + sample binding-report.json + output/, artifacts/ (behavior-tier, pack-gate, x64-*-gate reports with skip stats), generator source tree (hotspots + cross-cuts via grep), runtime marshalling core, git history (recent ~154 commits on 4 largest files since late 2025; heavy focus on Apple frameworks, existentials, x64/Intel, ARC/+1, CSM, KeyPaths; "AI-style" commit messages), unit test layout (strong EmitterTests/, MarshalerTests/, ParserTests/ (31 files), DemanglerTests/, ConcreteSpecializationEngineTests, EveryProtocolEmitterTests, etc.), and Nuke build/validation orchestration.
