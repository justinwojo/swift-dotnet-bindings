...
1. **HPKE construction surface** — the instance-method side now binds end-to-end (the `ClassifyConformerStructurally` NestedType gate was lifted in `3cd6d0f4`; `Seal`/`Open`/`ExportSecret` emit concrete overloads). The remaining blocker is the CSM specialization path not running for *initializers* carrying method-own generic type parameters, so all 10 `HPKE.Sender`/`HPKE.Recipient` inits drop. **Open** → [06 T2.2](06-remaining-work.md#t22--cryptokit-generic-remainders) (current root cause + fix direction live there).
...
3. **`EveryEntityProtocol` carrier unreachable on real RealityFoundation input** — the routing gate `HasClassSuperclassRequirement` (`EveryProtocolEmitter.cs`) never saw the `<Self : Entity>` genericSig constraint because `SwiftABIParser.cs` `CreateProtocolDecl` populated `InheritedProtocols` only from `node.Conformances`. **RESOLVED**: routing-gate fix shipped, the `EveryEntityProtocol : Entity` carrier emits, and the real-RealityKit `EntityTranslationGestureRecognizer.Entity` identity round-trip passes on sim + device (iPhone 13 NativeAOT, 2026-05-31). See [05 → entity-gesture device round-trip](05-residual-gaps.md#entity-gesture-device-round-trip).
