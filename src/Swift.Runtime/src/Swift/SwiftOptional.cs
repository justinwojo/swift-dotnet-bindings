...
public class SwiftOptional<T> : ISwiftObject, ISwiftStruct, IDisposable
{
    ...
    public unsafe PayloadBuffer<IntPtr> PayloadBuffer
    {
        ...
        {
            ThrowIfDisposed();
            // PayloadBuffer<IntPtr> performs a whole-word (8-byte) load. It is only valid when the
            // Optional payload fits in one machine word; larger payloads must go through
            // DangerousGetHandle(). The buffer itself is always allocated to at least one word
            // (_bufferAllocSize), so this word-load never over-reads the allocation.
