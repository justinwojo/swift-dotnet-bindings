...
public interface ISwiftObject : IDisposable
{
    ...
    /// Retained for callers explicitly managing legacy borrowed payloads. This controls finalization
    /// only: it does not prevent explicit Dispose or extend the source buffer's lifetime.
    /// <see cref="InteropServices.SwiftMarshal.MarshalCallbackArg{T}"/> takes an independent copy
    /// and no longer calls this seam. The default is a no-op for types without a separate payload;
    /// heap-backed wrappers override it to suppress finalization on their payload SafeHandle.
    /// </summary>
    void SuppressPayloadFinalizer() { }

    /// <summary>
    /// Arranges container-only cleanup for a caller-managed borrowed Move payload. A type with
    /// separate storage (such as SwiftString) frees that storage without destroying its borrowed
    /// value. This does not extend the source value's lifetime. The default suppresses finalizers
    /// for compatibility with types whose container cannot be separated from the value.
    /// Current callback marshalling takes independent copies and does not call this legacy seam.
    /// </summary>
