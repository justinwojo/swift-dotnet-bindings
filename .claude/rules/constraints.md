...
- **Validation cache invalidation**: `validate-libraries.sh` caches in a branch-specific `/tmp/binding-validation-<branch>/` dir. Use `rm -rf /tmp/binding-validation-$(git rev-parse --abbrev-ref HEAD | tr '/' '-')` before full validation when generator source has changed.
