...
- Scoped rules live in `.claude/rules/*.md` and use frontmatter `paths` to define applicability.
- Always read `AGENTS.md` and `CLAUDE.md` first, then load only scoped rule files whose `paths` match the current task scope.
...
- In-tree test apps (including the mixed ObjC+Swift one) set `IncludeSwiftBindingsRuntimeNative=false` on their Swift.Runtime project reference and let the Nuke harness inject the native runtime into the app bundle post-build. The native runtime ships as a `SwiftBindingsRuntime.framework` `NativeReference`, which does not propagate across a `ProjectReference`, so the in-tree builds opt out and inject the correct per-platform slice themselves.
