...
// MARK: - Existential-param + completion handler

/// Reproduces the bug that originally leaked an existential heap allocation
/// per call: a method whose non-closure parameter list contains an `any Protocol`
/// existential, paired with a trailing `@escaping (T) -> Void` completion handler.
/// The OLD generator emitted a duplicated `…Async` overload that allocated
/// `existentialContextHeap` and never freed it. The fix in
...
public final class Bug3CompletionFixture {
    ...
    /// `(any Protocol, @escaping (Int32) -> Void) -> Void` shape. Generator emits a
    /// callback overload (used to allocate + free the existential) plus a
