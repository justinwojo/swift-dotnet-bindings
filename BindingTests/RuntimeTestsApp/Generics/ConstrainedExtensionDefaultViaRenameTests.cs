...
public class ConstrainedExtensionDefaultViaRenameTests : TestBase
{
    ...
    // `KindType` (parent kept its `Kind` property). Independent of any conformance work.

    public void TestParentExposesRenamedNestedType()
    {
        using var rv = new DefaultRawValue(value: 1);
        using var kind = new ConstraintHost.KindType(rawValue: rv);
        using var host = new ConstraintHost(kind: kind);
        using var roundTrip = host.Kind;
        AssertEqual(1, roundTrip.RawValue.Value,
            "ConstraintHost.Kind property round-trips a renamed KindType payload");
    }

    // --- Interface declaration: the property tested by the runtime gate. Pre-fix this
    // is_check returns false because IConstrainedDefaulted is missing from KindType's
    // declared interfaces.

    public void TestRenamedNestedType_DeclaresProtocolInterface()
    {
        using var kind = new ConstraintHost.KindType(rawValue: new DefaultRawValue(value: 7));
        AssertTrue(kind is IConstrainedDefaulted,
            "ConstraintHost.KindType declares IConstrainedDefaulted in its interface list");
    }
    ...
    // `DefaultedCursor<KindType>` fails CS0311 — the binding-tests --compile-only gate
    ...
    {
        using var kind = new ConstraintHost.KindType(rawValue: new DefaultRawValue(value: 3));
        using var host = new ConstraintHost(kind: kind);
        ...
        AssertEqual("via:DefaultRawValue value=1", cursor.Describe(),
            "DefaultedCursor<KindType>.Describe() routes through the constrained-extension default");
    }
}
